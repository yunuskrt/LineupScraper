...
from config import MANAGERS_LINK_PATH, MATCH_PARSED_DATA_PATH
...
def save_link(link):
    ...
    with open(MANAGERS_LINK_PATH, "a", encoding=ENCODING) as f:
...
def download_links_from_index():
    ...
    if not os.path.exists(MANAGERS_LINK_PATH):
        with open(MANAGERS_LINK_PATH, "w", encoding=ENCODING) as f:
    ...
    # get the downloaded links and start page
        ...
        data = pd.read_csv(MANAGERS_LINK_PATH, sep="\t")
        if data.shape[0] == 0:
            start_page = 0
            downloaded_url_list = []
        else:
    ...
    with open(MATCH_PARSED_DATA_PATH, "r", encoding=ENCODING) as f:
        for line in f:
            ...
            homeManagerKeys = set(line_data['home']['manager'].keys())
            awayManagerKeys = set(line_data['away']['manager'].keys())
            if homeManagerKeys != manager_keys or awayManagerKeys != manager_keys:
                check = False
              
    if check:
        managers_data_set = set()
        with open(MATCH_PARSED_DATA_PATH, "r", encoding=ENCODING) as f:
            for line in f:
                line_data = json.loads(line)
                homeManager = line_data['home']['manager']
                awayManager = line_data['away']['manager']
                managers_data_set.add(homeManager['link'])
                managers_data_set.add(awayManager['link'])

        print(len(managers_data_set))
        
        for manager_link in managers_data_set:
            save_link(manager_link)
