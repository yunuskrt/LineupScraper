...
def check_match(match):
    ...
    match_keys = ['home', 'away', 'round', 'date', 'stadium','attendance', 'referee', 'id']
    if not checkRequiredObject(match, match_keys):
        ...
        if not checkRequiredObject(home, home_keys):
            ...
            if not checkOptionalInt(home['position']):
                error_list.append('home position is not optional int')
        ...
        if not checkRequiredObject(away, away_keys):
            ...
            if not checkOptionalInt(away['position']):
                error_list.append('away position is not optional int')
