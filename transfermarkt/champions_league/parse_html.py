...
def extract_content_from_page(file_path):
    ...
    try:
        ...
        homeBox = headerBox.find('div',{'class':'sb-team sb-heim'}) # homeName, homeLink
    ...
    try:
        ...
        halfScoreElement = scoreInfo.find('div',{'class':'sb-halbzeit'})
        
        if halfScoreElement.text == 'AET':
            matchRec['extraTime'] = True
        elif 'pens' in halfScoreElement.text.lower():
            matchRec['penalty'] = True
        else:
            halfScore = halfScoreElement.text[1:-1].split(':') # homeHalfScore, awayHalfScore
            matchRec['home']['halfScore'], matchRec['away']['halfScore'] = int(halfScore[0]), int(halfScore[1])

    except Exception as e:
        return {'type': None, 'msg': 'homeHalfScore, awayHalfScore or penalty Error'}
    ...
    try:
        awayBox = headerBox.find('div',{'class':'sb-team sb-gast'}) # awayName, awayLink
        awayLink = awayBox.find('a',{'class':'sb-vereinslink'})
        matchRec['away']['name'], matchRec['away']['link'] = awayLink.text, awayLink['href']
        
    except Exception as e:
        return {'type': None, 'msg': 'awayName or awayLink Error'}
    ...
    try:
        ...
        for player in home11Elements:
            ...
            number = None
            numberElement = player.find('div',{'class':'tm-shirt-number tm-shirt-number--large tm-shirt-number--bordered'})
            if numberElement is not None:
                numText = numberElement.text.translate(str.maketrans('', '', ' \n'))
                if numText.isdigit():
                    number = int(numText)
    ...
    try:
        ...
        for player in homeSubPlayerList:
            # [{number:int, name:string,  link: string, position: string, actions:[]}]
            number = None
            numberElement = player.find('div',{'class':'tm-shirt-number tm-shirt-number--small'})
            if numberElement is not None and numberElement.text.isdigit():
                number = int(numberElement.text)
